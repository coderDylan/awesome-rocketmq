...
/**
 * 集群状态（路由元数据）信息维护
 * 可以看到，这里使用Map来存储很多信息，而且大量使用可重入的读写锁来优化性能（读多写少的场景）
 *
 * 可能很多同学会疑惑，namesrv既然是集群，那集群状态一致性怎么解决？选主问题怎么处理？并没有看到Paxos/Raft相关的实现啊？
 * 其实Broker会向所有的namesrv节点上报心跳注册信息，这样每个namesrv节点都有完整信息，Product、Consumer随机选择1个namesvr节点即可
 */
public class RouteInfoManager {
    ...
    //topic和queue的路由表（消息发送者在负载均衡时会根据该路由表）
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                ...
                //维护filterServer（一个Broker上会关联多个FilterServer消息过滤服务器）
    ...
    private void createAndUpdateQueueData(final String brokerName, final TopicConfig topicConfig) {
        ...
        if (null == queueDataList) {
            //如果没有该Topic，则创建Topic，并放入topicQueueTable中
    ...
    /**
     * 扫描不活跃（失效）的Broker，释放链接，从列表中移出
     */
    public void scanNotActiveBroker() {
        Iterator<Entry<String, BrokerLiveInfo>> it = this.brokerLiveTable.entrySet().iterator();
        while (it.hasNext()) {
            Entry<String, BrokerLiveInfo> next = it.next();
            //Broker和Namesvr之间保持长链接心跳检测（由Broker定时上报，默认每个30s上报一次），维护最后更新时间
            long last = next.getValue().getLastUpdateTimestamp();
            //如果 最后更新时间 + 过期时间（默认2分钟）< 当前时间，或者通俗点理解"2分钟都没有上报心跳"，则视为该Broker节点已失效
